...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        def ffn_residual_func(x: torch.Tensor) -> torch.Tensor:
            return self.ls2(self.mlp(self.norm2(x))+1*self.adapter2(self.norm2(x)))  # 0.2 is the scaling factor for Parallel adapter
...
@register_model
def vit_base_patch16_224_fix(pretrained: bool = False, **kwargs) -> VisionTransformer:
