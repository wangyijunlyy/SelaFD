...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, ff_dim, dropout):
        ...
        self.drop = nn.Dropout(dropout)
        self.sample_drop_ratio = dropout
    ...
    def forward(self, x, mask):
        ...
        def attn_residual_func(x: Tensor) -> Tensor:
            return self.adapter1(self.proj(self.attn(self.norm1(x), mask)))

        def ffn_residual_func(x: Tensor) -> Tensor:
            return self.pwff(self.norm2(x))+1*self.adapter2(self.norm2(x))  # 0.2 is the scaling factor for Parallel adapter
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            x = x + self.drop(attn_residual_func(x))
            x = x + self.drop(ffn_residual_func(x))  # FIXME: drop_path2
        else:
